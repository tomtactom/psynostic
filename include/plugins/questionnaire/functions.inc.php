...
function questionnaireLoadActiveQuestionnaire(PDO $pdo) {
	$stmt = $pdo->prepare('SELECT id, slug, title, intro_text, standard_rules_json, scoring_config_json FROM questionnaires WHERE status = :status ORDER BY updated_at DESC, id DESC LIMIT 1');
...
function questionnaireLoadBySlug(PDO $pdo, $slug) {
	$stmt = $pdo->prepare('SELECT id, slug, title, intro_text, standard_rules_json, scoring_config_json FROM questionnaires WHERE slug = :slug AND status = :status LIMIT 1');
	$stmt->execute(array(
		':slug' => (string)$slug,
		':status' => 'active'
	));
...
function questionnaireLoadItems(PDO $pdo, $questionnaireId) {
	try {
		$stmt = $pdo->prepare('SELECT id, item_key, item_no, item_text, scale_type, likert_min, likert_max, is_reversed, subscale_key, is_required FROM questionnaire_items WHERE questionnaire_id = :questionnaire_id ORDER BY item_no ASC, id ASC');
		$stmt->execute(array(':questionnaire_id' => (int)$questionnaireId));
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	} catch (Throwable $e) {
		throw new RuntimeException('Migration fehlt: questionnaire_items.item_key wird benötigt. Bitte Datenbankschema aktualisieren.', 0, $e);
	}
}
...
function questionnaireCompleteSessionIdempotent(PDO $pdo, $sessionId, array $questionnaire, array $demographics, array $responses) {
	...
	try {
		...
		$scoringConfig = questionnaireResolveScoringConfig($questionnaire);
		if (!empty($scoringConfig['missing_configuration'])) {
			throw new RuntimeException(isset($scoringConfig['error_message']) ? (string)$scoringConfig['error_message'] : 'Fehlende Skalenkonfiguration auf Fragebogenebene.');
		}

		$itemStmt = $pdo->prepare('SELECT id, item_no, item_key, likert_min, likert_max, is_reversed, subscale_key FROM questionnaire_items WHERE questionnaire_id = :questionnaire_id');
		$itemStmt->execute(array(':questionnaire_id' => (int)$questionnaire['id']));
		$items = $itemStmt->fetchAll(PDO::FETCH_ASSOC);
		$itemMap = array();
